    ...
    public class AccountsPayableReportController : Controller
    {
        ...
        public async Task<IActionResult> GenerateLiquidationReportExcelFile(int purchaseOrderId, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var fileName = $"{purchaseOrder.Supplier!.SupplierName}_{purchaseOrder.PurchaseOrderNo}_//SELECTED DATE_Liquidation_Report_{DateTimeHelper.GetCurrentPhilippineTime():yyyyddMMHHmmss}.xlsx";
                var stream = new MemoryStream();
                await package.SaveAsAsync(stream, cancellationToken);
                stream.Position = 0;
                return File(stream, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", fileName);
