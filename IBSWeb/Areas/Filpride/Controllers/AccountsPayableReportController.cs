...
namespace IBSWeb.Areas.Filpride.Controllers
{
    ...
    public class AccountsPayableReportController : Controller
    {
        ...
        public async Task<IActionResult> GenerateTradeSupplierFuelReportExcelFile(DateOnly monthDate, CancellationToken cancellationToken)
        {
            if (!ModelState.IsValid)
            {
                TempData["warning"] = "Please input date range";
                ...
                foreach (var receivingReports in receivingReportsGroupBySupplier)
                {
                    ...
                    foreach (var item in receivingReports)
                    {
                        ...
                        var netOfVatAmount = item.PurchaseOrder!.VatType == SD.VatType_Vatable
                            ? cvTradePayment?.CheckVouchers != null ? NetOfVatOrZero(item.Amount) : 0
                            : cvTradePayment?.CheckVouchers != null ? item.Amount : 0;

                        var taxPercent = cvTradePayment?.CheckVouchers != null ? item.TaxPercentage : 0;

                        var withHoldingTaxAmount = item.PurchaseOrder.TaxType == SD.TaxType_WithTax &&
                                                   cvTradePayment?.CheckVouchers != null
                        ...
                        foreach (var checkVoucher in (cvTradePayment?.CheckVouchers ?? Enumerable.Empty<dynamic>()).DefaultIfEmpty())
                        {
                            ...
                            var balance = item.Amount - (amountPaid + withHoldingTaxAmount);
                            ...
                            worksheet.Cells[row, col].Value = checkVoucher != null ? netOfVatAmount : "";
                            worksheet.Cells[row, col].Style.Numberformat.Format = currencyFormat;
                            col++;
                            worksheet.Cells[row, col].Value = checkVoucher != null ? withHoldingTaxAmount : "";
                            worksheet.Cells[row, col].Style.Numberformat.Format = currencyFormat;
                            col++;
