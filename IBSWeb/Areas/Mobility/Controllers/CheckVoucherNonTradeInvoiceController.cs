        ...
        public async Task<IActionResult> Edit(CheckVoucherNonTradeInvoicingViewModel viewModel, IFormFile? file, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                #region --Audit Trail Recording
                    ...
                    var ipAddress = HttpContext.Connection.RemoteIpAddress?.ToString();
                    FilprideAuditTrail auditTrailBook = new(existingModel.EditedBy, $"Edited check voucher# {existingModel.CheckVoucherHeaderNo}", "Check Voucher", ipAddress, companyClaims);
                    await _dbContext.AddAsync(auditTrailBook, cancellationToken);
                ...
                if (model != null)
                {
                    if (model.CanceledBy == null)
                    {
                        ...
                        #region --Audit Trail Recording
                            ...
                            var ipAddress = HttpContext.Connection.RemoteIpAddress?.ToString();
                            FilprideAuditTrail auditTrailBook = new(model.CanceledBy, $"Canceled check voucher# {model.CheckVoucherHeaderNo}", "Check Voucher", ipAddress, companyClaims);
                            await _dbContext.AddAsync(auditTrailBook, cancellationToken);
                ...
                try
                {
                    if (model.VoidedBy == null)
                    {
                        ...
                        #region --Audit Trail Recording
                            ...
                            var ipAddress = HttpContext.Connection.RemoteIpAddress?.ToString();
                            FilprideAuditTrail auditTrailBook = new(model.VoidedBy, $"Voided check voucher# {model.CheckVoucherHeaderNo}", "Check Voucher", ipAddress, companyClaims);
                            await _dbContext.AddAsync(auditTrailBook, cancellationToken);
        ...
        public async Task<IActionResult> Printed(int id, int? supplierId, CancellationToken cancellationToken)
        {
            ...
            if (!cv.IsPrinted)
            {
                #region --Audit Trail Recording
                    ...
                    var ipAddress = HttpContext.Connection.RemoteIpAddress?.ToString();
                    FilprideAuditTrail auditTrailBook = new(printedBy, $"Printed original copy of check voucher# {cv.CheckVoucherHeaderNo}", "Check Voucher", ipAddress, companyClaims);
                    await _dbContext.AddAsync(auditTrailBook, cancellationToken);
        ...
        public async Task<IActionResult> EditPayrollInvoice(CheckVoucherNonTradeInvoicingViewModel viewModel, IFormFile? file, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                #region --Audit Trail Recording
                    ...
                    var ipAddress = HttpContext.Connection.RemoteIpAddress?.ToString();
                    FilprideAuditTrail auditTrailBook = new(existingHeaderModel.CreatedBy, $"Edited check voucher# {existingHeaderModel.CheckVoucherHeaderNo}", "Check Voucher", ipAddress, companyClaims);
                    await _dbContext.AddAsync(auditTrailBook, cancellationToken);
